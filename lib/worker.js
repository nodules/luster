...
Worker = ClusterProcess.create(function Worker() {
    ...
    this.on('initialized', function () {
        this._broadcastEvent('initialized');
...
/**
 * Turns worker in 'ready' state, if it's already 'initialized'
...
Worker.prototype.ready = function () {
    if (this._initialized) {
        this._broadcastEvent('ready');
    } else {
        this.on('initialized', broadcastEvent.bind(this, 'ready'));
    }
}
