 ...
 * @property {Boolean} [triggerReadyStateManually=false]
 *      If true, worker won't come to 'ready' state until automatically.
 *      Use require('luster').ready() in your worker
...
WorkerWrapper = EventEmitterEx.create(function WorkerWrapper(master, options) {
    ...
    this.options = {
        ...
        triggerReadyStateManually: options.triggerReadyStateManually
    };
    ...
    master.on('worker ready', this._onWorkerReady.bind(this));
    master.on('worker loaded', this._onWorkerLoaded.bind(this));
 ...
 * @fires WorkerWrapper#loaded
 * @private
 */
WorkerWrapper.prototype._onWorkerLoaded = function(worker) {
 ...
 * @private
 */
WorkerWrapper.prototype._onWorkerReady = function(worker) {
    if (this._worker && worker.id === this._worker.id && !this.ready) {
        this.emit('ready');
    }
};
...
/**
 * event:WorkerWrapper#loaded handler
 * @fires WorkerWrapper#ready
 * @private
 */
WorkerWrapper.prototype._onLoaded = function() {
    if (!this.options.triggerReadyStateManually) this.emit('ready');
};
...
WorkerWrapper.prototype.stop = function() {
    ...
    legacy.setImmediate(function() {
        // state can be changed before fucntion call
        if (self.isRunning()) {
