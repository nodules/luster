 ...
 *      Time (in ms) to wait from 'disconnect' event to 'exit' before `worker.kill` call with SIGTERM
 *      If evaluates to `false` then stopTimeout will not set.
 * @property {Number} [killTimeout=false]
 *      Time (in ms) to wait from 'disconnect' event to 'exit' before `worker.kill` call with SIGKILL
 *      If evaluates to `false` then stopTimeout will not set.
...
class WorkerWrapper extends EventEmitterEx {
        ...
        setImmediate(() => {
            ...
            if (this.isRunning()) {
                this.remoteCallWithCallback({
                    ...
                    callback: () => {
                        // Worker could die while suspend was in-flight,
                        // i.e. on endless loop, and _onStateStopped will drop _worker
    ...
    _scheduleForceStop() {
        ...
        // In some cases default SIGTERM is not enough to stop worker if it stacked seriously
        // We want to be sure we are able to stop rogue worker no matter what
        if (this.options.killTimeout && !this.killTimeout) {
            this.killTimeout = setTimeout(() => {
                this.killTimeout = null;

                if (this._worker !== null) {
                    console.log('force kill attempt');
    ...
    _cancelForceStop() {
        if (this.stopTimeout) {
            throw new Error('cancel sigterm');
            clearTimeout(this.stopTimeout);
            this.stopTimeout = null;
        }

        if (this.killTimeout) {
            throw new Error('cancel sigkill');
